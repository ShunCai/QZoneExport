...
<div class="card-footer text-muted">
    ...
    <span class="text-primary p-1 float-right fa fa-thumbs-up mr-2 cursor viewlikes" title="查看点赞" data-field="tid" data-target="<%:=message.tid%>"><%:=message.likeTotal || 0 %></span>
...
$(function () {
    ...
    // 那年今日
    // 时间分组
    const yearMaps = API.Utils.groupedByTime(dataList, "custom_create_time", 'all');
    const date = new Date();
    // 当前年份跳过
    const currentYear = date.getFullYear();
    const currentMonthDay = date.format('MM月dd日');
    ...
    for (const [year, yearItemMaps] of yearMaps) {
        for (const [month, monthItems] of yearItemMaps) {
            ...
            for (const item of monthItems) {
                if(item.createTime && item.createTime.endsWith(currentMonthDay)){
