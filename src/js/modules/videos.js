...
API.Videos.addDownloadTasks = (videos, module_dir, source, FILE_URLS) => {
    ...
    // 下载相对目录
    module_dir = module_dir || 'Videos/Images';
    ...
    for (const video of videos) {
        ...
        API.Utils.newDownloadTask(video.custom_pre_url, module_dir, video.custom_pre_filename, video);
        ...
        if (!FILE_URLS.has(video.custom_url)) {
            // 添加下载任务
            API.Utils.newDownloadTask(video.custom_url, module_dir, video.custom_filename, source || video);
...
API.Videos.exportToHtml = async (videos) => {
    ...
    try {
        ...
        for (const [year, year_items] of year_maps) {
            ...
            const yearFile = await API.Common.writeHtmlofTpl('videos', null, QZone.Videos.ROOT + "/" + year + ".html");
